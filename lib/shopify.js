...
export async function fetchCart(id) {
  ...
  const cart = response.data.cart ? response.data.chart : [];

  return cart;
}

export async function addItemsToCart(cartId, lines) {
  const query = `
        mutation {
  ...
  const cart = response.data.cartLinesAdd
...
export async function deleteItemsFromCart(cartId, lineIds) {
  ...
  const cart = response.data.cartLinesRemove
